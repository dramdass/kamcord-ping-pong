...
{
    new Firebase("https://crackling-heat-5835.firebaseio.com/ping-pong/doubles-matches/").on("value", handleMatches);
    new Firebase("https://crackling-heat-5835.firebaseio.com/ping-pong/doubles-pending/").on("value", handlePending);
    initClickHandlers();
});
...
function submitNewMatch()
{
    ...
    console.log(match);
    var matchRef = new Firebase("https://crackling-heat-5835.firebaseio.com/ping-pong/doubles-pending/").push(match);
...
function acceptPendingMatch(key, match)
{
    ...
    {
        var ref = new Firebase("https://crackling-heat-5835.firebaseio.com/ping-pong/");
        ...
        ref.authWithPassword({"email":email, "password": password}, function(error, authData)
            {
                ...
                else
                {
                    ...
                    var pingpongRef = new Firebase("https://crackling-heat-5835.firebaseio.com/ping-pong");
    ...
    {
        new Firebase("https://crackling-heat-5835.firebaseio.com/ping-pong/doubles-pending").child(key).remove();
        $("#auth_background").fadeOut(200);
    });
}
